...
class ClusterClient():
    def __init__(self,grad_path_list:List[str],weight_path_list:List[str],req_name:List[str],
        ...
        self.weight_tensor=VctorAndParas().clientpthlist2tensor(weight_path_list,req_name)
        ...
        self.rou=1.0
        self.z=VctorAndParas().clientpthlist2tensor(weight_path_list,req_name)
...
def all_long_cluster(weight_one_vector:torch.tensor,grad_one_vector:torch.tensor,epoch=5,long=3000,p=0.01,gama=10)->torch.tensor:
    ...
    i=0
    grad_vector=[]
    clustergrad=ClusterGradients([],[],long=long,epoch_num=epoch,gama=gama)
    t1=time.time()

    while vector_long:
        if i%(250/epoch)==0:
            clustergrad=ClusterGradients([],[],long=long_step,epoch_num=epoch,gama=gama)#empty the cach
        ...
        if vector_long==long_step:
            ...
            start_long=start_long-long_step
            long_step=1000
            clustergrad=ClusterGradients([],[],long=long_step,epoch_num=epoch,gama=gama)
            ...
            clustergrad=ClusterGradients(weight_one_vector[start_long:start_long+long_step],
                                        ...
                                        long=vector_long,epoch_num=epoch,gama=gama)
            gradients,_=clustergrad.get_cluster_gradients()
            grad_vector.append(gradients.cpu().detach().numpy())
            start_long=start_long+long_step
    t2=time.time()
    ...
    print("-"*3+"\033[1;33m compress done: {} s \033[0m".format(t2-t1)+"-"*3)
...
class MyPlot():
    ...
    def plot_global_acc(file_path):
        global_loss=[]
        global_acc=[]
        with open(file_path,"r") as f:
            lines=f.read()
            loss=re.findall("global loss: (\d+\.\d+)",lines)
            acc=re.findall("global accuracy: (\d+\.\d+)",lines)
        global_loss=[float(i) for i in loss]
        global_acc=[float(i) for i in acc]
        x=list(range(len(global_loss)))
        p1,=plt.plot(x,global_loss)
        p2,=plt.plot(x,global_acc)
        plt.legend([p1,p2],["loss","acc"],loc=1)
        plt.show()

    def compara(dir_path,type="acc"):
...
class VctorAndParas():
    ...
    def vector2params(self,net,vector_list:list,name_list:list)->dict:
        net_state={}
    ...
    def clientpthlist2tensor(self,path_list:List[str],req_name:List[str]=[])->List[torch.tensor]:
