...
def read_nifti_seg_file(filepath,by_torch=True):
    # Read file
    img = nib.load(filepath)
    # img = img.get_fdata().astype(np.float32)
...
def random_split_data_loader(dataset):
    ...
    trainset, testset = torch.utils.data.random_split(dataset, [train_size, test_size],generator=torch.Generator().manual_seed(0))
