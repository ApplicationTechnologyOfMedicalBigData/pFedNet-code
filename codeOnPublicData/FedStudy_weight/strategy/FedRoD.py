...
class FedRoDStrategy():
    def __init__(self,net) -> None:
        self.net=net
    def tell_client_calculate_global_layers(self,server_round,clients_proxy):
        ...
        for client_name in clients_proxy:
            if server_round==1:
                ...
                clients_proxy[client_name]["client_data"].send2client["Fedloss"]="FedRoD_BRLoss"
    ...
    def tell_client_caculate_personal_layers(self,server_round,clients_proxy):
        print("-"*10+"\033[1;33m personal parameters \033[0m"+"-"*10)
        sp=myfn.SPath(server_round,None)
        for client_name in clients_proxy:
            sp.name=client_name
            send_model_path=sp.send_model_path()
            ...
            clients_proxy[client_name]["client_data"].send2client["Fedloss"]="FedRoD_PERLoss"
        return clients_proxy
    
    def tell_client_caculate_eval(self,clients_proxy):
