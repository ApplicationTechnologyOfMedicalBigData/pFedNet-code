    ...
    def calculate_u(self,fit_results,server_round):
        ...
        for result in fit_results:
            ...
            if weight_count==0:
                ...
                weight_avg=weight_avg+vector
                weight_count+=1
            name_list.append(result["name"])
        weight_avg=weight_avg/weight_count
        
        u_matrix=torch.zeros((weight_count,weight_count),device=DEVICE)
        for i in range(weight_count):
            for j in range(i+1,weight_count):
                sig=self.dericate(weight_list[i],weight_avg)
                u_matrix[i,j]=sig
        u_matrix=u_matrix+u_matrix.T
        u_matrix=u_matrix*alpha
        u_namda=(1-torch.sum(u_matrix,dim=0))*torch.eye(weight_count,device=DEVICE)
